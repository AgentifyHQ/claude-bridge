...
# Or with just (justfile is auto-generated by setup)
...
└── justfile                # optional, for just users (created if not present)
```
...
- Creates a `justfile` if one doesn't exist (optional convenience wrapper)
